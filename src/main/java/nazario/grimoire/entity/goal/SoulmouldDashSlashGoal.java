...
public class SoulmouldDashSlashGoal extends Goal {
    ...
    public SoulmouldDashSlashGoal(AvengingSoulmouldEntity entity) {
        this.mould = entity;
    ...
    public void tick() {
        ...
        if(ticks == 10 || ticks == 13 || ticks == 15) {
            mould.playSound(ModSounds.ENTITY_SOULMOULD_ATTACK, 1f, 1f);
            List<LivingEntity> entities = mould.getWorld().getEntitiesByClass(LivingEntity.class, mould.getBoundingBox().expand(4, 3, 4), livingEntity -> livingEntity != mould && livingEntity != mould.getOwner() && !(livingEntity instanceof AvengingSoulmouldEntity smould && smould.getOwner() == mould.getOwner()) && mould.distanceTo(livingEntity) <= 4 + livingEntity.getWidth() / 2 && livingEntity.getY() <= mould.getY() + 3);
