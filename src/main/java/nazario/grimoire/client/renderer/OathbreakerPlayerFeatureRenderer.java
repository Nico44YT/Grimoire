...
public class OathbreakerPlayerFeatureRenderer<T extends PlayerEntity, M extends EntityModel<T>> extends FeatureRenderer<T, M> {
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light,
        ...
        if (entity.getInventory().contains(ModItems.OATHBREAKER.getDefaultStack())
                && !entity.getStackInHand(Hand.MAIN_HAND).isOf(ModItems.OATHBREAKER)
                && !entity.getStackInHand(Hand.OFF_HAND).isOf(ModItems.OATHBREAKER)) {

            Arm mainArm = ((ClientPlayerEntity)entity).getMainArm();
